...
namespace lime { namespace spoopy {
    ...
    void PipelineShader::CreateVertexShader(Bytes bytes) {
        shaders.push_back(std::make_unique<Shader>(device, bytes, VK_SHADER_STAGE_VERTEX_BIT));
    }

    void PipelineShader::CreateFragmentShader(Bytes bytes) {
        shaders.push_back(std::make_unique<Shader>(device, bytes, VK_SHADER_STAGE_FRAGMENT_BIT));
    }

    void PipelineShader::CreateGeometryShader(Bytes bytes) {
        shaders.push_back(std::make_unique<Shader>(device, bytes, VK_SHADER_STAGE_GEOMETRY_BIT));
    }

    void PipelineShader::CreateTessellationControlShader(Bytes bytes) {
        shaders.push_back(std::make_unique<Shader>(device, bytes, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT));
    }

    void PipelineShader::CreateTessellationEvaluationShader(Bytes bytes) {
        shaders.push_back(std::make_unique<Shader>(device, bytes, VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT));
    }
}}